          ...
          ...)
{
  ...
  if("constant"%in%names(z$coefficients)){
    RSS<-sum((ans$residuals)^2)#rdf
    TSS <- sum((z$model[,1] - mean(z$model[,1]))^2)
    k<-ncol(z$model)-2
    n<-nrow(z$model)
    ans$r.squared<-1-(RSS/TSS)
    ans$adj.r.squared<-ans$r.squared/((n-1)/(n-k))
    f.old<-as.formula(paste(z$terms[[2]],"~",paste(names(z$coefficients)[!names(z$coefficients)%in%"constant"], collapse = " + ")))
    ans$fstatistic[[1]]<-as.numeric(summary(lm(f.old,z$model))$fstatistic[[1]])
    ans$fstatistic[[2]]<-p - df.int-1
    ans$fstatistic[[3]]<-rdf
         }
